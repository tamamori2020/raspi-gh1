...
#define LED1        22
#define LED2        25
#define BLINK_TIME  10000   //点滅時間（期間）ms

/* プロトタイプ宣言 */
void *Thread1Hz(void *arg); //1Hz信号
void *Thread2Hz(void *arg); //2Hz信号
void *Thread3Hz(void *arg); //3Hz信号

int main(void){
    int t;
    pthread_t thread0,thread1,thread2;  //スレッドの宣言
    ...
    if(pthread_create(&thread2, NULL, Thread3Hz, (void *)t) != 0){ 
        fprintf(stderr,"Error pthread_create.\n");
        exit(EXIT_FAILURE);
    }
...
//関数名  void *Thread3Hz(void *arg)
//引数    arg　時間(ms)
//戻り値  なし
//概要    引数の時間(ms)だけ、LED2を3Hzで点滅させる
void *Thread3Hz(void *arg){
    int i,t;
    t = (int) arg;
    for(i = 0; i < (int) t/333; i++){
        printf("LED2: #%d\n",i);
