...
/* プロトタイプ宣言 */
void *serialReceive(void *arg); //受信のスレッド
...
int main (void){
    ...
    if(pthread_create(&threadSerial, NULL, serialReceive, NULL) != 0){
    ...
    while(1){
        if(strcmp(g_cmd,cmdOld) != 0){
            if(strcmp(g_cmd,"start") == 0){         //点滅: start
                ...
                printf("\nError: %s is unknown.\n",g_cmd);
                int i = 0;
                while(g_cmd[i] != '\0'){    //一文字毎の表示
                    serialPrintf(fdSri,"%c(0x%02x) ",g_cmd[i],g_cmd[i]); 
                    printf("%c(0x%02x) ",g_cmd[i],g_cmd[i]); fflush(stdout);
                    i++;
                }
                serialPrintf(fdSri,"\r\n");printf("\n");
            }
...
//関数名  void *serialReceive(void *arg)
//引数    なし
//戻り値  なし
    ...
    while(g_cmd_LED0 != END){
        ...
        if(c > 0){
            for(i = 0; i < c; i++){
                ...
                if( rxdData == '\r'){           //Enter(CR:\r)なら、
                    str[k++] = '\0';            //nullを加えて文字列にする
                    strcpy(g_cmd, str);
                    k=0;
                }else{
                    str[k++] = rxdData;         //文字列に受信データを代入
                }
            }
        }
        usleep(1000);           //CPU使用率の抑制のため
    }
    serialPrintf(fdSri,"End of serialReceive.\r\n");
    printf("End of serialReceive.\n");
    pthread_exit(NULL);
}
